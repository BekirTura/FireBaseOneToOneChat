 ...
 */
public class ProgressWheelByDirectional extends View {
...
//    private int contourColor = 0xAA000000;
    private int contourColor = 0x00000000;
...
//    int progress = 0;
    int startProgress = 90, endProgress = 0;
    boolean isSpinning = false,isIncreaseStart = true;
     ...
     * @param attrs
     */
    public ProgressWheelByDirectional(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void parseAttributes(TypedArray a) {
        ...
        barLength = (int) a.getDimension(R.styleable.ProgressWheel_barLengths,barLength);
...
//            canvas.drawArc(circleBounds, progress - 90, barLength, false, barPaint);
            canvas.drawArc(circleBounds, startProgress, endProgress - 90, false, barPaint);
        } else {
//            canvas.drawArc(circleBounds, 90, progress, false, barPaint);
            canvas.drawArc(circleBounds, startProgress, endProgress, false, barPaint);
        }
    ...
    private void scheduleRedraw() {
    	startProgress = startProgress - spinSpeed;
    	/*if(startProgress < -90) {
    		startProgress = 90;
    	}*/
    	if(startProgress < 0) {
    		startProgress = 360;
    	} else if (startProgress == 269) {
    		startProgress = 90;
    	}
    	endProgress = endProgress + spinSpeed;
    	endProgress ++;
    	if(endProgress > 360) {
    		endProgress = 0;
    	}
...
//        progress = 0;
    ...
    public void incrementProgress() {
        isSpinning = false;
        if(isIncreaseStart) {
        	isIncreaseStart = false;
        	startProgress--;
//        	Log.i("Start progress", " = "+startProgress);
        	if(startProgress < 0) {
        		startProgress = 360;
        	} else if (startProgress == 269) {
        		startProgress = 90;
        	...
        	isIncreaseStart = true;
        	endProgress ++;
        	endProgress ++;
//        	Log.i("End progress", " = "+endProgress);
        	if(endProgress > 360) {
        		endProgress = 0;
        	}
        }
    ...
    public void setProgress(int i) {
        isSpinning = false;
        if(i%2 == 0) {
        	startProgress = startProgress - (i/2);
        	endProgress = endProgress + (i/2);
            endProgress = endProgress + endProgress;
            /*Log.i("Start progress"," = "+startProgress);
            Log.i("End progress"," = "+endProgress);*/
        } else {
        	i--;
